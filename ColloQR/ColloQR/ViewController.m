...
- (void)viewDidLoad
{
    ...
    _voice = [AVSpeechSynthesisVoice voiceWithLanguage:@"fr-FR"];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self startRunning];
    NSLog(@"ScanRect %@", NSStringFromCGRect([_previewLayer metadataOutputRectOfInterestForRect:_scanView.frame]));
    _metadataOutput.rectOfInterest = [_previewLayer metadataOutputRectOfInterestForRect:_scanView.frame];
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        ...
        [newBarcodes enumerateObjectsUsingBlock:^(BarCode *barcode, BOOL *stop) {
            ...
            utterance.rate = AVSpeechUtteranceMinimumSpeechRate + ((AVSpeechUtteranceMaximumSpeechRate - AVSpeechUtteranceMinimumSpeechRate) * [[NSUserDefaults standardUserDefaults] floatForKey:@"speed"]);
            utterance.volume = [[NSUserDefaults standardUserDefaults] floatForKey:@"volume"];
            utterance.pitchMultiplier = [[NSUserDefaults standardUserDefaults] floatForKey:@"pitch"];
