    ...
    [[_session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (!error) {
            ...
            if (((NSHTTPURLResponse *)response).statusCode == 200) {
                NSString *stringText = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
                dispatch_async(dispatch_get_main_queue(), ^{
                    [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
                    self.textView.text = stringText;
                });
            } else {
                // Handle bad response
            }
...
- (IBAction)done:(id)sender
{
    ...
    if (![_textView.text isEqualToString:@""]) {
        ...
        // Upload
        NSURLSessionUploadTask *uploadTask = [_session uploadTaskWithRequest:request fromData:noteContents completionHandler:^(NSData *data, NSURLResponse *theResponse, NSError *error) {
